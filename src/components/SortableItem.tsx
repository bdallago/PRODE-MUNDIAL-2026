...
export const SortableItem: React.FC<{ id: string, team: string, index: number, disabled?: boolean }> = ({ id, team, index, disabled = false }) => {
  ...
  return (
    <div
      ...
      {...(!disabled ? attributes : {})}
      {...(!disabled ? listeners : {})}
      className={`flex items-center gap-3 p-3 mb-2 rounded-md border ${isDragging ? 'bg-purple-50 border-purple-300 shadow-lg scale-[1.02]' : 'bg-white border-gray-200'} ${disabled ? 'bg-gray-50' : 'cursor-grab active:cursor-grabbing hover:bg-gray-50'} transition-colors duration-200`}
    >
      {!disabled && (
